// https://raw.githubusercontent.com/vtex-apps/add-to-cart-button/a82400c526ac8d98e4a751dbcc434cb79c0c289a/react/modules/assemblyOptions.ts
...
export function sumAssembliesPrice(assemblyOptions: Record<GroupId, ProductTypes.AssemblyOptionItem[]>): number {
  ...
  return assembliesGroupItems.reduce((sum, groupItems) => {
    const groupPrice = groupItems.reduce<number>((groupSum, item) => {
      const childrenPrice: number = item.children ? sumAssembliesPrice(item.children) : 0;
    ...
    for (const item of items) {
      ...
      if (item.children) {
        ...
        assemblyInputValuesKeys = assemblyInputValuesKeys.filter((inputValueKey) =>
          handledInputValues.includes(inputValueKey)
        );
      ...
      const { options: childrenOptions, assemblyOptions: childrenAssemblyOptions } = childrenAddedData ?? {
        options: undefined,
      ...
      if (quantity >= initialQuantity && quantity > 0) {
        added.push({
          ...
          item: {
            ...
            quantity,
            sellingPriceWithAssemblies: (item.price as number) + sumAssembliesPrice(item.children ?? {}),
            id: item.id,
            ...(childrenAssemblyOptions ? { assemblyOptions: childrenAssemblyOptions } : {}),
          },
        });
      }
      ...
      const addedChildrenCount = childrenAddedData ? childrenAddedData.options.length : 0;
      ...
      if (quantity !== initialQuantity || addedChildrenCount > 0) {
        options.push({
          ...
          seller: item.seller,
          ...(childrenOptions && childrenOptions.length > 0 ? { options: childrenOptions } : {}),
        });
      }
    }
  }
